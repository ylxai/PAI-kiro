...
`;
    ...
    if (agentConfig.hooks) {
      for (const eventType in agentConfig.hooks) {
        for (const kiroHook of agentConfig.hooks[eventType]) {
          hooks.push(this.convertFromKiroCLIHook(kiroHook, eventType));
        }
      }
    }
  ...
  private convertFromKiroCLIHook(kiroHook: KiroCLIHook, eventType: string): HookEvent {
  ...
  async createCustomAgent(config: Partial<KiroCLIAgentConfig>): Promise<string> {
    ...
    let existing: Partial<KiroCLIAgentConfig> = {};
    try {
      const content = await fs.readFile(agentPath, 'utf-8');
      existing = JSON.parse(content);
    } catch {}

    // Merge prompt: append new prompt after existing content (e.g. Algorithm context)
    const mergedPrompt = config.prompt
      ? existing.prompt
        ? `${existing.prompt}\n\n${config.prompt}`
        : config.prompt
      : existing.prompt;
